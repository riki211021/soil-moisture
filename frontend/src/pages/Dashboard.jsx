...
export default function Dashboard() {
  ...
  const chartData = {
    labels: history.map((i, index) => {
      ...
      // hanya label pertama yang ada tanggal
      if (index === 0) {
        return `${day} ${time}`;
      }
  ...
  const PumpCard = ({ id, mode }) => (
    <div className="bg-white rounded-xl shadow border border-green-100 p-6">
      <div className="flex justify-between mb-4">
        <h3 className="font-semibold text-lg text-green-900">Pompa {id}</h3>
...
`}
      ...
      <div className="flex gap-2">
        <button disabled={mode === "auto"} onClick={() => controlPump(id, 1)} className="flex-1 bg-green-700 hover:bg-green-600 text-white py-2 rounded-lg disabled:opacity-40">
          Nyalakan
        </button>

        <button disabled={mode === "auto"} onClick={() => controlPump(id, 0)} className="flex-1 border border-green-200 py-2 rounded-lg hover:bg-green-50 disabled:opacity-40">
          Matikan
        </button>
      </div>
    </div>
    ...
    <div className="h-screen w-screen bg-gradient-to-br from-green-50 via-white to-green-50 flex flex-col p-6">
      ...
      <div className="flex justify-between items-center mb-6 border-b pb-4">
        <h1 className="text-4xl font-light text-green-900">
          Dashboard <span className="font-semibold text-green-600">Kebun</span>
        </h1>
      ...
      <div className="flex-1 grid lg:grid-cols-[340px,1fr] gap-8 overflow-hidden">
        ...
        <div className="flex flex-col gap-6 overflow-y-auto">
          {latest && (
            <>
              <h2 className="text-xl font-semibold text-green-900">Kelembaban Tanah</h2>

              <div className="grid grid-cols-2 gap-4">
                <div className={`bg-gradient-to-br ${getSensorColor(latest.status_1)} text-white p-6 rounded-xl shadow`}>
                  <p className="text-xs uppercase opacity-70">Sensor 1</p>

                  <p className="text-4xl font-bold mt-1">{latest.moisture_1}%</p>

                  <p className="text-sm mt-2">{latest.status_1}</p>
                </div>

                <div className={`bg-gradient-to-br ${getSensorColor(latest.status_2)} text-white p-6 rounded-xl shadow`}>
                  <p className="text-xs uppercase opacity-70">Sensor 2</p>

                  <p className="text-4xl font-bold mt-1">{latest.moisture_2}%</p>

                  <p className="text-sm mt-2">{latest.status_2}</p>
                </div>
              </div>
            </>
          )}

          <h2 className="text-xl font-semibold text-green-900">Kontrol Pompa</h2>

          <div className="flex flex-col gap-4">
            <PumpCard id={1} mode={pump1Mode} />
            <PumpCard id={2} mode={pump2Mode} />
          </div>
        </div>
        ...
        <div className="flex flex-col">
          <h2 className="text-xl font-semibold text-green-900 mb-4">Riwayat Kelembaban</h2>
      ...
      <div className="text-center text-sm text-gray-500 mt-4">© {new Date().getFullYear()} Dashboard Monitoring Kebun</div>
    </div>
  );
}
